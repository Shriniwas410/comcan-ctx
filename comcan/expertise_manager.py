...
def generate_manifesto(repo_root: Path) -> str:
    """Synthesize all expertise into a high-level human-readable manifesto.

    Args:
        repo_root: Git repository root.

    Returns:
        Markdown-formatted master architecture document.
    """
    all_records = query_all(repo_root)
    if not any(all_records.values()):
        return "# AI Architecture Manifesto\n\nNo architecture rules recorded yet."

    lines: list[str] = [
        "# 🧠 AI Architecture Manifesto",
        "",
        "This document is the centralized 'Main Brain' for this repository.",
        "It contains verified architectural rules, patterns, and lessons learned.",
        "",
        "---",
        "",
    ]
    ...
    for domain, records in sorted(all_records.items()):
        ...
        lines.append(f"## 🏛️ Domain: {domain.upper()}")
        lines.append("")
        ...
        for record_type in sorted(RECORD_TYPES):
            ...
            lines.append(f"### {icon} {record_type.title()}s")
            for rec in recs:
                lines.append(f"- **{rec.content}**")
                if rec.description:
                    lines.append(f"  - *Context:* {rec.description}")
                if rec.resolution:
                    lines.append(f"  - *Resolution:* {rec.resolution}")
            lines.append("")

    lines.extend([
        "---",
        f"\n*Generated by ComCan v{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}*",
    ])
    return "\n".join(lines)


def import_from_branch(repo_root: Path, branch: str) -> dict[str, int]:
    """Import expertise records from another Git branch without merging code.

    Args:
        repo_root: Git repository root.
        branch: The branch name to import from.

    Returns:
        Dict mapping domain names to the number of NEW records imported.
    """
    stats: dict[str, int] = {}
    expertise_dir = _expertise_dir(repo_root)
    
    # Get the list of expertise files on the target branch
    try:
        result = subprocess.run(
            ["git", "ls-tree", "-r", "--name-only", branch, ".comcan/expertise/"],
            cwd=str(repo_root),
            capture_output=True,
            text=True,
            check=True
        )
        files = [f.strip() for f in result.stdout.splitlines() if f.endswith(".jsonl")]
    except subprocess.CalledProcessError:
        return stats

    for remote_path_str in files:
        domain = Path(remote_path_str).stem
        
        # Read the file content from the other branch
        try:
            content_result = subprocess.run(
                ["git", "show", f"{branch}:{remote_path_str}"],
                cwd=str(repo_root),
                capture_output=True,
                text=True,
                check=True
            )
            remote_lines = content_result.stdout.splitlines()
        except subprocess.CalledProcessError:
            continue

        # Get local records to avoid duplicates
        local_records = {r.id for r in query(repo_root, domain)}
        ...
        for line in remote_lines:
            ...
            try:
                data = json.loads(line)
                if data["id"] not in local_records:
                    # Append strictly new records to the local domain file
                    domain_file = _domain_path(repo_root, domain)
                    lock = _acquire_lock(domain_file)
                    try:
                        with open(domain_file, "a", encoding="utf-8") as f:
